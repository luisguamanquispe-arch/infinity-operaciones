...
import { getClienteWithService, serializeService } from "@/lib/cliente-app/service";
...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json({
      service: serializeService(data.cliente, data.service),
    });
  } catch (err) {
    if (err instanceof Error && err.message === "UNAUTHORIZED") {
      return NextResponse.json({ error: "No autorizado" }, { status: 401 });
    }
    console.error("[cliente/service]", err);
