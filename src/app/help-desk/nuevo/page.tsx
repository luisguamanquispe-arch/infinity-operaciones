...
export default function NuevoSoporteRemotoPage() {
  ...
  useEffect(() => {
    void fetch("/api/help-desk/catalogo")
      ...
      .then((d) => {
        ...
        if (d.session?.id) setForm((f) => ({ ...f, operadorId: d.session.id }));
      })
      .catch(() => {});
  }, []);
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      void fetch(`/api/help-desk/catalogo?q=${encodeURIComponent(busquedaCliente.trim())}`)
        .then((r) => r.json())
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      router.push(`/help-desk/${data.ticket.id}`);
    } catch (err) {
        ...
        <form onSubmit={submit} className="bg-white rounded-xl border p-4 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <label className="text-sm space-y-1">
              ...
              <select
                ...
                className="w-full px-3 py-2 border rounded-lg"
              >
                {SR_ESTADOS.filter((e) => e !== "ESCALADO").map((e) => (
            ...
            <label className="text-sm space-y-1">
              ...
              <select
                ...
                className="w-full px-3 py-2 border rounded-lg"
              >
                {SR_PRIORIDADES.map((p) => (
                  <option key={p} value={p}>
                    {SR_PRIORIDAD_LABELS[p]}
                  </option>
                ))}
              </select>
            </label>
          ...
          <label className="block text-sm space-y-1">
            ...
            <textarea
              ...
              onChange={(e) => setField("accionesRealizadas", e.target.value)}
              className="w-full px-3 py-2 border rounded-lg"
            />
          </label>
          ...
          >
            ...
            {saving ? "Guardando…" : "Crear soporte"}
          </button>
        </form>
      </main>
    </div>
